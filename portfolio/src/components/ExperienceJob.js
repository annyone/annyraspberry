...
//
// Сетка из двух колонок: логотип и всё остальное. Описание и достижения
// стоят во второй колонке — их левый край совпадает с названием компании.
...
export default function ExperienceJob({ job }) {
  const { t } = useLanguage();
  const { company, logo, link, description, achievements } = job;
  ...
  const hasBody = Boolean(description) || items.length > 0;

  return (
    <article className="grid grid-cols-[2.5rem_1fr] gap-x-4 gap-y-4 xl:gap-x-5">
      ...
      {hasBody && (
        <div className="col-span-2 max-w-3xl space-y-4 md:col-span-1 md:col-start-2">
          ...
          {items.length > 0 && (
            <MarkerList items={items} className={`space-y-2 ${variants.p.className}`} />
          )}
        </div>
      )}
    </article>
