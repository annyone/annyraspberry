...
export default function AxelnacCase({ project }) {
  const { t } = useLanguage();
    ...
    <Layout title={`${title} — ${t('site.author')}`}>
      ...
      <Section title={t('pages.axelnac.tasks.title')} className="gap-4">
        <div className="grid gap-4 grid-cols-1 xl:grid-cols-2">
          {Array.isArray(tasks) &&
            tasks.map((task, i) => (
              <Banner key={i} emoji={task.icon} title={task.title} text={task.text} />
            ))}
        </div>
      </Section>

      <Section title={t('pages.axelnac.done.title')} className="gap-4">
        <MarkerList className="space-y-2" items={done} />
      </Section>

      <Section title={t('pages.axelnac.problems.title')} className="gap-4">
        <MarkerList className="space-y-2" items={problems} />
      </Section>
      ...
      <Section title={t('pages.axelnac.before.title')} className="gap-4">
        <Article>
          ...
          <Image
            ...
            sources={[{ srcSet: '/images/axelnac/step1-2x.webp', media: '(min-width: 1024px)' }]}
        ...
        <Article last>
          ...
          <Image
            ...
            sources={[{ srcSet: '/images/axelnac/step2-2x.webp', media: '(min-width: 1024px)' }]}
            alt={t('pages.axelnac.before.altBefore')}
            shadow
          />
        </Article>
      ...
      <Section title={t('pages.axelnac.after.title')} className="gap-4">
        <Article>
          ...
          <Image
            ...
            sources={[{ srcSet: '/images/axelnac/empty-2x.webp', media: '(min-width: 1024px)' }]}
            alt={t('pages.axelnac.before.altBefore')}
            shadow
          />
        </Article>
        <Article>
          ...
          <Image
            ...
            sources={[{ srcSet: '/images/axelnac/started-2x.webp', media: '(min-width: 1024px)' }]}
            alt={t('pages.axelnac.before.altBefore')}
            shadow
          />
        </Article>
